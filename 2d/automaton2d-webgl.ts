...
// Fullscreen quad vertex shader (GLSL ES 3.00). Passes through NDC positions and maps them to
// texture coordinates in [0, 1]: a_position in [-1, 1] → v_uv in [0, 1].
...
// Display fragment shader (GLSL ES 3.00): samples the state texture (R channel = color id in [0,1]),
// decodes the id, looks up RGB in the 1D palette texture. Flips v_uv.y so that
// state row 0 appears at the top of the screen (same as Canvas 2D).
...
export class Automaton2DWebGL {
	...
	private fillRandomAndDraw(): void {
		for (let i = 0; i < this.stateIds.length; i++) {
			this.stateIds[i] = Math.floor(Math.random() * this.colorsCount)
		}
